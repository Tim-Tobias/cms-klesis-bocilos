        ...
        <div class="card-body">
          <form action="{{ "/dashboard/home/" . $data_image->id }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group col-12 mb-3">
                ...
                <input type="text" class="form-control @error('name') is-invalid @enderror" id="basicInput" value="{{ $data_image->name }}" name="name" placeholder="Enter email" fdprocessedid="m3kdzr">
                @error('name')
            ...
            <div class="input-group mb-3 col-12">
                ...
                <select class="form-select @error('order') is-invalid @enderror" name="order" id="inputGroupSelect01">
                    @for ($i = 0; $i < $order; $i++)
                    <option selected={{ $data_image->order == ($i + 1) }} value="{{ $i + 1 }}">{{$i + 1}}</option>
                    @endfor
                </select>
                @error('order')
                    <span class="invalid-feedback" role="alert">
